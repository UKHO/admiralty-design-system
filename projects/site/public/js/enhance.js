...
for (var idx2 = 0; idx2 < examples.length; idx2++) {
  ...
  if (htmlView) {
    ...
    var escapedHtml = document.createElement('div').appendChild(document.createTextNode(exampleHtml)).parentNode
      .innerHTML;
...
for (var idx3 = 0; idx3 < popupLinks.length; idx3++) {
  ...
  if (!noContentInjection || noContentInjection === 'false') {
    popupLink.addEventListener('click', function(evt) {
      ...
      var contentElem =
        injectContentFrom === 'parent'
          ? evt.target.parentElement.parentElement
          : document.querySelector(injectContentFrom);
