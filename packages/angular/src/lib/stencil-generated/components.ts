...
@ProxyCmp({
  inputs: ['accessibleLabel', 'actionText', 'colour', 'enableCardEvent', 'heading', 'height', 'href', 'linkText', 'suppressRedirect', 'width']
})
@Component({
...
export class AdmiraltyTableHeaderCell {
  ...
  constructor(c: ChangeDetectorRef, r: ElementRef, protected z: NgZone) {
    c.detach();
    this.el = r.nativeElement;
    proxyOutputs(this, this.el, ['admiraltySortChange']);
  }
}
...
new `direction` value (`'ascending'`, `'descending'`, or `'none'`).
   */
  admiraltySortChange: EventEmitter<CustomEvent<{ direction: 'none' | 'ascending' | 'descending' }>>;
}
