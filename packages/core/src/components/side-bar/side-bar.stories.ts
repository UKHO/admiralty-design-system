...
export const NoFooterItems: Story = {
  render: args => html` <div style="background-color: #ccc; display: flex; height: 100vh;">
    <admiralty-side-bar-wrapper>
      <admiralty-side-bar label="Application navigation" show-logo="${args.showLogo}" side-bar-width="${args.sideBarWidth}">
        <admiralty-side-bar-item active="${false}" href="/contents" suppress-redirect="${args.suppressRedirect}" icon="list-alt-outline" item-text="Contents"></admiralty-side-bar-item>
        <admiralty-side-bar-item active="${true}" href="/bookmarks" suppress-redirect="${args.suppressRedirect}" icon="bookmark-outline" item-text="Bookmarks"></admiralty-side-bar-item>
        <admiralty-side-bar-item active="${false}" href="/recents" suppress-redirect="${args.suppressRedirect}" icon="history" item-text="Recents"></admiralty-side-bar-item>
      </admiralty-side-bar>
    </admiralty-side-bar-wrapper>
  </div>`,
};
