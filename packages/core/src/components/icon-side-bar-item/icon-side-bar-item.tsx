 ...
 */
@Component({
  tag: 'admiralty-icon-side-bar-item',
  styleUrl: 'icon-side-bar-item.scss',
  scoped: true,
})
export class IconSideBarItemComponent {
  private internalId: number = ++nextId;

  @Element() el!: HTMLAdmiraltyIconSideBarItemElement;
   ...
   * `onIconSideBarItemClick` event to use a navigation router and prevent a full page reload when navigating.
   */
   ...
   */
  @Event() iconSideBarItemClick: EventEmitter<string>;
  ...
  onToggle() {
    ...
    if (this.expanded) {
      this.el.dispatchEvent(new CustomEvent('icon-side-bar-item-active', {
  ...
  handleMouseDown(id: string) {
    this.active = true;
    const allItems = document.querySelectorAll('admiralty-icon-side-bar-item');
  ...
  handleClick(ev: MouseEvent): CustomEvent<string> {
    ...
    return this.iconSideBarItemClick.emit(this.href);
  }

  render() {
    return (
      <Host data-icon-side-bar-item-id={'icon-side-bar-item-' + this.internalId}>
        <li>
          <a id={"icon-side-bar-item-anchor-" + this.internalId}
             class="icon-side-bar-item-link"
             href={this.href}
             onMouseDown={() => this.handleMouseDown("icon-side-bar-item-anchor-" + this.internalId)}
