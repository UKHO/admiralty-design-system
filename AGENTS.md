...
- Website is static-exported Next app (`output: "export"`, `trailingSlash: true` in `packages/website/next.config.mjs`).
...
- Release pipeline uploads build artifacts for core/angular/react and runs `auto shipit` (`.github/workflows/actions/release-packages/action.yml`).
...
- **Cursor**: `.cursor/rules/` is currently empty; do not assume repo-specific auto-loaded rules.
